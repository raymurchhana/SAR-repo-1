...
def shrink_iso(px, py, tau):
    ...
    scale = np.maximum(1.0, mag/tau)
    return px/scale, py/scale

def deblur_tv_admm(b, k, lam=0.02, rho=1.2, iters=200, tol=1e-4):
    n, m = b.shape
    # FFT precomputation
    ...
    for k_iter in range(iters):
        ...
        # Convergence check
        if np.linalg.norm(x - prev_x) / (np.linalg.norm(prev_x) + 1e-8) < tol:
...
x_rec = deblur_tv_admm(b, psf, lam=0.015, rho=2.0, iters=300)
...
axs[2].set_title("ADMM-TV (optimized)")
axs[2].axis('off')
plt.tight_layout()
plt.show()
